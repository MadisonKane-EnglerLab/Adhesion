%% Required m-files for ShearAnalysis_rev11.m:
%     fancyPlot.m
%     readnd2.m  %download comsol shears 
%% Required MATLAB Toolboxes for ShearAnalysis_rev11.m:
...
folder = 'D:\Human Breast Cancer Sample Project\Patient 5 (surgery on 3-31-2023)\dPPFC\P.2 Post Digest\Unsorted (Left and right side pooled)\2023-04-08';
...
function I3 = ProcessImage(I)
    ...
    for i=1:numel(idx)-1
        if i==numel(idx)-1
            J = I(idx(i):end,:);
            I2(idx(i):end,:) = imadjust(J,[0 0.2],[],2);
        else
            J = I(idx(i):idx(i+1),:);
            I2(idx(i):idx(i+1),:) = imadjust(J,[0 0.2],[],2);
        end
    end
    I3 = imbinarize(I2);
end
...
elseif cLines == 2    
   eqn = 'p*exp(-(x/m1)^b1)+(1-p)*exp(-(x/810.4)^1.4154)';
    ft = fittype( eqn, 'independent', 'x', 'dependent', 'y' );
    opts = fitoptions( 'Method', 'NonlinearLeastSquares' );
    opts.Display = 'Off';
    opts.Lower = [0 0 0];
    opts.StartPoint = [2 100 0.5];
    opts.Upper = [15 1000 1];
end
